<template>
  <div class="record-data-wrapper">
    <div
      class="listed-records"
      :style="{ height: `${listedRecordContainerHeight}px` }"
    >
      <div class="list-wrapper" v-if="listedRecords.length > 0">
        <div
          ...
          :class="{
            'list-item': true,
            'selected-list-item': selectedRecordId === record.id
          }"
        ...
        >
            ...
            <i
              v-if="
                record.protocol === 'https' &&
                  !hostsWithHttps.includes(record.host)
              "
              class="el-icon-lock lock-icon"
              @click.prevent.stop="enableHttps4Host(record.host)"
            ></i>
            <i
              v-if="
                record.protocol === 'https' &&
                  hostsWithHttps.includes(record.host)
              "
              class="el-icon-unlock lock-icon"
              @click.prevent.stop="disableHttps4Host(record.host)"
            ></i>
            <i
              v-if="record.protocol !== 'https'"
              class="el-icon-link lock-icon"
            ></i>
          </span>
          ...
          <span
            class="host-column column"
            :title="`${record.protocol}://${record.host}`"
          >
      ...
      <div class="no-record-msg" v-if="listedRecords.length === 0">
        当前没有网络数据！
      </div>
    </div>
    <div
  ...
  methods: {
    ...
    handleCursorMove(e) {
      ...
      if (this.isCursorMove) {
        if (
          this.listedRecordContainerHeight + diffY >= 100 &&
          this.listedRecordContainerHeight + diffY <= 600
        ) {
  ...
  .listed-records {
    ...
    .list-wrapper {
      ...
      .list-item {
        ...
        .column i.el-icon-lock {
          color: #777;
          cursor: pointer;
        }
        .column i.el-icon-unlock {
          color: rgb(103, 194, 58);
          cursor: default;
        }
        .column i.el-icon-link {
          color: rgb(103, 194, 58);
          cursor: default;
        }
        .id-column {
