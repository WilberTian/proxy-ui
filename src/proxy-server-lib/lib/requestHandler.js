...
function getUserReqHandler(userRule, recorder) {
  ...
  return function(req, userRes) {
    ...
    */
    ...
    logUtil.printLog(
      color.green(`received request to: ${req.method} ${host}${path}`)
    )
    ...
    const prepareRequestDetail = () => {
      const options = {
        ...
        port:
          urlPattern.port || req.port || (/https/.test(protocol) ? 443 : 80),
        path,
        method: req.method,
        headers: req.headers
      }
    ...
    const sendFinalResponse = finalResponseData => {
      ...
      const transferEncoding =
        resHeader['transfer-encoding'] || resHeader['Transfer-Encoding'] || ''
      const contentLength =
    ...
    co(fetchReqData)
      ...
      .then(() => {
        ...
        try {
          ...
          const contentEncoding =
            reqHeaders['content-encoding'] || reqHeaders['Content-Encoding']
      ...
      .then(
        co.wrap(function*() {
          const userModifiedInfo =
            (yield userRule.beforeSendRequest(
              Object.assign({}, requestDetail)
            )) || {}
          const finalReqDetail = {}
          ;['protocol', 'requestOptions', 'requestData', 'response'].map(
            key => {
      ...
      .then(
        co.wrap(function*(userConfig) {
          if (userConfig.response) {
            ...
            const remoteResponse = yield fetchRemoteResponse(
              ...
              userConfig.requestData,
              {
                dangerouslyIgnoreUnauthorized:
                  reqHandlerCtx.dangerouslyIgnoreUnauthorized,
                chunkSizeThreshold
              }
            )
            return {
            ...
            throw new Error(
              'lost response or requestOptions, failed to continue'
            )
          }
        })
      )
      ...
      .then(
        co.wrap(function*(responseData) {
          if (responseData._directlyPassToRespond) {
            ...
            responseData.response.body instanceof CommonReadableStream
          ) {
            // in stream mode
            return responseData
          } else {
            // TODO: err etimeout
            return (
              (yield userRule.beforeSendResponse(
                Object.assign({}, requestDetail),
      ...
      .catch(
        co.wrap(function*(error) {
          ...
          // call user rule
          try {
            const userResponse = yield userRule.onError(
              Object.assign({}, requestDetail),
              error
            )
            if (
      ...
      .then(responseInfo => {
        ...
        resourceInfo.resBody =
          responseInfo.body instanceof CommonReadableStream
            ? '(big stream)'
            : responseInfo.body || ''
      ...
      .catch(e => {
        logUtil.printLog(
          color.green('Send final response failed:' + e.message),
          logUtil.T_ERR
        )
      })
  }
...
function getWsHandler(userRule, recorder, wsClient, wsReq) {
  ...
  try {
    ...
    const wsUrl = `${serverInfo.protocol}://${
      serverInfo.hostName
    }${serverInfoPort}${serverInfo.path}`
    ...
    proxyWs.onclose = event => {
      logUtil.debug(
        `proxy ws closed with code: ${event.code} and reason: ${event.reason}`
      )
      const targetCloseInfo = getCloseFromOriginEvent(event)
      wsClient.readyState !== 3 &&
        wsClient.close(targetCloseInfo.code, targetCloseInfo.reason)
    }
    ...
    wsClient.onclose = event => {
      logUtil.debug(
        `original ws closed with code: ${event.code} and reason: ${
          event.reason
        }`
      )
      const targetCloseInfo = getCloseFromOriginEvent(event)
      proxyWs.readyState !== 3 &&
        proxyWs.close(targetCloseInfo.code, targetCloseInfo.reason)
    }
  } catch (e) {
