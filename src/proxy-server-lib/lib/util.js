...
function getProxyUIHome() {
  const home = path.join(getUserHome(), '/.proxy-ui/')
  if (!fs.existsSync(home)) {
    fs.mkdirSync(home)
  }
  return home
}
__exports.getProxyUIHome = getProxyUIHome

__exports.getProxyUIPath = function(pathName) {
  const home = getProxyUIHome()
...
__exports.getProxyUITmpPath = function() {
  const targetPath = path.join(os.tmpdir(), 'proxy-ui', 'cache')
  if (!fs.existsSync(targetPath)) {
  ...
  if (fs.existsSync(dirPath)) {
    ...
    if (ifClearFolderItself) {
      try {
        ...
        throw new Error(
          'could not remove directory (code ' + e.code + '): ' + dirPath
        )
      }
    }
  }
